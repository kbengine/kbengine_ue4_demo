...
ENTITYDEF_PROPERTY_REGISTER(Monster, entityNO);

// 由于与底层某些名称冲突，所以这里使用了一个别名防止冲突
ENTITYDEF_PROPERTY_ALIAS_REGISTER(Monster, name, name_);

ENTITYDEF_PROPERTY_REGISTER(Monster, uid);
ENTITYDEF_PROPERTY_REGISTER(Monster, utype);
ENTITYDEF_PROPERTY_REGISTER(Monster, modelID);
ENTITYDEF_PROPERTY_REGISTER(Monster, modelScale);
